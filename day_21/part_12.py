...
def find_new_sequences(string, options):
    ...
    for i in range(1, len(string)):
        for j in range(len(sequences) - 1, -1, -1):
            for k in options:
                # print(string[i-1], string[i], sequences[j], k)
...
def find_route_directional():
    pass

def main():
    start_numeric = [3, 2]
    start_directional = [0, 2]
    codes = load_data("input.csv")
    numerical_keypad = make_numerical_keypad()
    direction_keypad = make_directional_keypad()
    ...
    for i in codes:
        ...
        i = "A" + i
        first_sequences = []
        all_second_sequences = []
        all_final_sequences = []
        lengths_first = []
        for j in range(0, len(i) - 1):
            # print(f"{j} -------------------------------------------------------------, dus {i[j:j+2]}")
            # code_locations = turn_codes_into_locations(i, numerical_keypad)
            ...
            for k in first_sequence:
                ...
                for m in range(0, len(k) - 1):
                    ...
                    for n in second_sequence:
                        ...
                        # print(f"we kijken naar {i[j:j+2]}, first sequence {first_sequence}, k {k}, m {m}, n {n} en de lengte is {sum_lengths_final}")
                        # print(sum_lengths_final)
                    lengths_second.append(min(sum_lengths_final))
                    # print(f"lengths second = {lengths_second}")
                sum_lengths_first.append(sum(lengths_second))
                # print(f"lengths first sum = {sum_lengths_first}")
            lengths_first.append(min(sum_lengths_first))
            # print(lengths_first)
        sum_lengths.append(sum(lengths_first))
    ...
    # print(sum_lengths)
        ...
        # print(lengths_final)
                    ...
                    # mid_final_sequences.append(final_sequences)

            # all_second_sequences.append(second_sequences)
            # all_final_sequences.append(mid_final_sequences)
        ...
        # print(all_second_sequences)
        # print(all_final_sequences)

            # second_sequence = []
            # for j in first_sequence:
            #     second_sequences = find_new_sequences("A" + j, directional_option)
            #     for k in second_sequences:
            #         second_sequence.append(k)

            # print(first_sequence)

        # final_sequence = []
        # for m in second_sequence:
        #     final_sequences = find_new_sequences("A" + m, directional_option)
        #     for n in final_sequences:
        ...
        # first_directional_locations = turn_codes_into_locations(first_sequence, direction_keypad)
        # first_directional_sequence = find_route_numeric(first_directional_locations, start_directional)

        # second_directional_locations = turn_codes_into_locations(first_directional_sequence, direction_keypad)
        # second_directional_sequence = find_route_numeric(second_directional_locations, start_directional)

    
    #     print(f"{i} \n{first_sequence} \n{first_directional_sequence} \n{second_directional_sequence}")
    #     print(len(second_directional_sequence), int(i[0:-1]))
